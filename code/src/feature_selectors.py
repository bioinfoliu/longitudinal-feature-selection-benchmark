...
def _glmmlasso_scores(
    ...
    with tempfile.TemporaryDirectory(prefix="ctsnn_glmmlasso_") as temp:
        ...
        completed = subprocess.run(
            ...
            check=False, capture_output=True, text=True, timeout=600, env=environment,
        )
...
def rank_features(
    ...
    method_limit = min(prefilter_limit, 40) if method in {"LongGroupLasso", "geeVerse"} else prefilter_limit
    method_limit = min(method_limit, 20) if method in {"glmmLasso", "PGEE"} else method_limit
